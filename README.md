...
カメラ映像は原則として端末内で処理され、外部サーバーには送信されません。人物画像・顔画像・動画は保存しません。顔認証も行いません。MVP 2ではMediaPipeの無料WASMと公式モデルを初回起動時にCDNから読み込みますが、カメラ映像や骨格データは送信しません。次段階でこれらを`public/models`へローカル配置し、主要機能の完全オフライン動作を目指します。
...
| 項目 | 使用技術 | 費用 | 外部通信 |
| --- | --- | --- | --- |
| 姿勢推定 | MediaPipe Tasks Vision | 無料 | 現在は公式CDNからWASM/modelを取得。ローカル化予定 |
